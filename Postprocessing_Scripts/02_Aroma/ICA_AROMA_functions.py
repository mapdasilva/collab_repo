...
def denoising_afni(inFile, mask, outFile, confounds_path, lowpass, highpass, polort):
    """ This function denoisise the the functional data non aggressively using AFNI's 3dTProject
    ...
    def create_denoise_workflow(name="denoising"):
        workflow = pe.Workflow(name=name)

        #Define nodes
        inputnode = pe.Node(util.IdentityInterface(fields=[
            'in_file',         # Smoothed BOLD file
            'mask_file',       # Brain mask
            'confounds_file',  # Nuisance regressor file (e.g., from fMRIPrep)
            'bandpass',        # Low-pass frequency for bandpassing
            'polort'           # Polynomial order for detrending
        ]), name='inputnode')

        outputnode = pe.Node(util.IdentityInterface(fields=['out_file']), name='outputnode')
     
        calc_mean = pe.Node(afni.TStat(
            options='-mean',
            outputtype='NIFTI_GZ'
        ), name='calc_mean')

        scale_robust = pe.Node(afni.Calc(
            expr='c * min(200, a/b*100) * step(a) * step(b)',
            outputtype='NIFTI_GZ'
        ), name='scale_robust')

        # Node for nuisance regression (3dtproject)
        tproject = pe.Node(afni.TProject(
            outputtype='NIFTI_GZ',
            num_threads=10 # Set a reasonable default
        ), name='nuisance_regression')


        # Connect workflow 
        workflow.connect(inputnode, 'in_file', calc_mean, 'in_file')
        workflow.connect(inputnode, 'mask_file', calc_mean, 'mask')
        workflow.connect(inputnode, 'in_file', scale_robust, 'in_file_a')
        workflow.connect(calc_mean, 'out_file', scale_robust, 'in_file_b')
        workflow.connect(inputnode, 'mask_file', scale_robust, 'in_file_c')

        # Connect inputs to the regression node (tproject)
        workflow.connect(scale_robust, 'out_file', tproject, 'in_file')
        workflow.connect(inputnode, 'mask_file', tproject, 'mask')
        workflow.connect(inputnode, 'confounds_file', tproject, 'ort')
        workflow.connect(inputnode, 'bandpass',tproject,'bandpass')
        workflow.connect(inputnode, 'polort', tproject, 'polort')
        workflow.connect(tproject, 'out_file', outputnode, 'out_file')
        
        return workflow

    main_workflow = pe.Workflow(name='main_workflow')
    main_workflow.base_dir = os.path.dirname(outFile)
    denoise_wf = create_denoise_workflow()

    # --- 3. Set the inputs for the sub-workflow ---
    denoise_wf.inputs.inputnode.in_file = inFile
    denoise_wf.inputs.inputnode.mask_file = mask
    denoise_wf.inputs.inputnode.confounds_file = confounds_path
    denoise_wf.inputs.inputnode.bandpass = (highpass, lowpass)
    denoise_wf.inputs.inputnode.polort = polort

    # ExportFile node ---  
    ef = pe.Node(nio.ExportFile(), name="export")
    ef.inputs.out_file = outFile # Use 'target_file' for ExportFile

    # Connect the sub-workflow's output to the ExportFile node ---
    main_workflow.connect(denoise_wf, 'outputnode.out_file', ef, 'in_file')

    # --- 6. Run the workflow ---
    main_workflow.run()

def create_confounds(melmix, motionIDX, confounds_path):
    import os
    import pandas as pd
    output_path=confounds_path.replace('_confounds.txt','-aroma_confounds.txt')
    if not os.path.isfile(output_path):   
        confounds_df=pd.read_csv(confounds_path)
        melodic_df=pd.read_csv(melmix,sep='\\s+', header=None)
        aromanoise_df=melodic_df.iloc[:,motionIDX-1]
        all_confounds_df=pd.concat([confounds_df, aromanoise_df], axis=1)
        all_confounds_df.to_csv(output_path, index=False, header=None)
    
    return output_path



def register2MNI(database_path, subject, inFile, bold_metadata, outFile, omp_nthreads=16, mem_gb=20):
    """ This function registers an image to MNI152 T1 2mm.

    Parameters
    ---------------------------------------------------------------------------------
    inFile:     Full path to the data file (nii.gz) which has to be registerd to MNI152
    outFile:    Full path of the output file
    database_path: fMRIPrep Output folder
    subject:  Subject ID
    """
    import os
    from fmriprep.workflows.bold.apply import init_bold_volumetric_resample_wf
    ...
    func_base=os.path.join(database_path, subject, f'sub-{subject}','func',f'sub-{subject}_task-ISC_')
    anat_base=os.path.join(database_path, subject, f'sub-{subject}','anat',f'sub-{subject}_')
    bold_dict={'bold_mask_native': func_base + 'desc-brain_mask.nii.gz',
           'boldref2anat':func_base + 'from-boldref_to-T1w_mode-image_desc-coreg_xfm.txt' ,
           'anat2mni152nlin6asym': anat_base + 'from-T1w_to-MNI152NLin6Asym_mode-image_xfm.h5'}

    mni6_mask = str(get_template( 'MNI152NLin6Asym',
                resolution=2,
                desc='brain',
                suffix='mask',
    ...
    bold_MNI6_wf = init_bold_volumetric_resample_wf(
                    ...
                    mem_gb={'resampled': mem_gb},
                    jacobian=False,
                    name='register')
    
    bold_MNI6_wf.inputs.inputnode.bold_file=inFile
    bold_MNI6_wf.inputs.inputnode.bold_ref_file = bold_dict['bold_mask_native']
    bold_MNI6_wf.inputs.inputnode.target_mask = mni6_mask
    bold_MNI6_wf.inputs.inputnode.target_ref_file = mni6_mask
    bold_MNI6_wf.inputs.inputnode.boldref2anat_xfm = bold_dict['boldref2anat']
    bold_MNI6_wf.inputs.inputnode.anat2std_xfm = bold_dict['anat2mni152nlin6asym']
    bold_MNI6_wf.inputs.inputnode.resolution = '02'
    
    ef=pe.Node(interface=ExportFile(), name="export")
    ef.inputs.out_file=outFile
    
    main_workflow.connect(bold_MNI6_wf, 'outputnode.bold_file', ef,'in_file')


    main_workflow.run()

def register2MNI_old(database_path, subject, inFile, bold_metadata, outFile, omp_nthreads=16, mem_gb=20):
    """ This function registers an image to MNI152 T1 2mm.
    ...
    func_base=os.path.join(database_path, subject, f'sub-{subject}','func',f'sub-{subject}_task-ISC_')
    fmap_base=os.path.join(database_path, subject, f'sub-{subject}','fmap',f'sub-{subject}_')
    anat_base=os.path.join(database_path, subject, f'sub-{subject}','anat',f'sub-{subject}_')
    bold_dict={'bold_mask_native': func_base + 'desc-brain_mask.nii.gz',
           'boldref2fmap': func_base + 'from-boldref_to-auto00000_mode-image_xfm.txt',
           'fmap_ref': fmap_base + 'fmapid-auto00000_desc-magnitude_fieldmap.nii.gz',
           'fmap_coeff': fmap_base + 'fmapid-auto00000_desc-coeff_fieldmap.nii.gz',
           'boldref2anat':func_base + 'from-boldref_to-T1w_mode-image_desc-coreg_xfm.txt' ,
           'anat2mni152nlin6asym': anat_base + 'from-T1w_to-MNI152NLin6Asym_mode-image_xfm.h5'}
    ...
    bold_MNI6_wf.inputs.inputnode.target_mask = mni6_mask
    bold_MNI6_wf.inputs.inputnode.target_ref_file = mni6_mask
    bold_MNI6_wf.inputs.inputnode.boldref2fmap_xfm = bold_dict['boldref2fmap']
    bold_MNI6_wf.inputs.inputnode.fmap_ref = [bold_dict['fmap_ref']]
    bold_MNI6_wf.inputs.inputnode.fmap_coeff = [bold_dict['fmap_coeff']]
    bold_MNI6_wf.inputs.inputnode.fmap_id = 'fmap'
    bold_MNI6_wf.inputs.inputnode.boldref2anat_xfm = bold_dict['boldref2anat']
    bold_MNI6_wf.inputs.inputnode.anat2std_xfm = bold_dict['anat2mni152nlin6asym']
    bold_MNI6_wf.inputs.inputnode.resolution = '02'
